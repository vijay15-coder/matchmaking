...
export default function MatchMaker(){
  ...
  // Download filtered matches as Excel
  const downloadExcel = () => {
    ...
    const data = filteredMatches.map((match, idx) => ({
      'Match #': match.matchNumber,
      'Player 1': match.player1.name,
      'Player 1 Master': match.player1.master || '—',
      'Player 1 Belt': match.player1.belt || '—',
      'Player 1 Age': match.player1.age || '—',
      'Player 1 Weight': match.player1.weight || '—',
      'Player 2': match.player2.name,
      'Player 2 Master': match.player2.master || '—',
      'Player 2 Belt': match.player2.belt || '—',
      'Player 2 Age': match.player2.age || '—',
      'Player 2 Weight': match.player2.weight || '—',
      'Player 1 Marks': match.player1Marks !== null && match.player1Marks !== undefined ? match.player1Marks : '—',
      'Player 2 Marks': match.player2Marks !== null && match.player2Marks !== undefined ? match.player2Marks : '—',
      'Winner': match.winner || '—',
    ...
    setMsg(`✅ Downloaded ${filteredMatches.length} matches as Excel!`)
    setMsgType('success')
    setTimeout(() => setMsg(''), 3000)
  }
        ...
        <div style={{background: '#f0f7ff', padding: 15, borderRadius: 8, marginBottom: 15, border: '2px solid #17a2b8'}}>
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12}}>
            ...
            <div>
              ...
              <button onClick={() => {setFilters({status: '', player1Name: '', player2Name: '', master: '', matchType: ''}); setCurrentPage(1)}} style={{width: '100%', padding: '8px 10px', borderRadius: 6, background: '#6c757d', color: 'white', border: 'none', cursor: 'pointer', fontWeight: 600}}>Clear Filters</button>
            </div>
          </div>
        </div>
      )}
